...
comments = """
 ...
 - Also, you can specify numbers as either 'X-Y' between X and Y seconds, or 'A:B-X:Y' for
         clips between (A minutes, B seconds) to (X minutes, Y seconds) in the video.
 
 - Example usage of script:
 
     \'python clip.py inputvideo_filename.mp4 3:05-4:16 12:1-12:45 -o some_output_filename.mp4\'
...
def clip(args, inputfile, outputfile, separate=False):
    clips = []
    for time in args:
        parsed = [t for t in time.split("-") if (isFloat(t) or (":" in t and all([isFloat(m) for m in t.split(":")])))]
        if len(parsed) != 2:
